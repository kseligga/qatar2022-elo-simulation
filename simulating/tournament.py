...
# getting elo ratings
f = open("copiedwebsite.txt", "r", encoding="utf8")
content = f.read()
teams = []
ratings = []
...
for i in range(1, 33):
    result1 = result[i].split(' rating-cell')
    result2 = result1[0].split('"')
    teams.append(result2[2])
    ratings.append(int(result1[1][3:7]))

# preparing data
ratings = [x for _, x in sorted(zip(teams, ratings))]
names = sorted(teams)
grouppies = ['C', 'D', 'F', 'G', 'G', 'F', 'E', 'F', 'D', 'A', 'B', 'D', 'E', 'H', 'B', 'E', 'C', 'F', 'A', 'C', 'H',
             'A', 'C', 'A', 'G', 'H', 'E', 'G', 'D', 'B', 'H', 'B']
...
def create():  # creating setup for every tournament is faster than deep copying every time
    # likely can be optimized, takes about 1/4 of whole simulation time
    teams = []
    for i in range(32):
        rating = ratings[i]
        name = names[i]
